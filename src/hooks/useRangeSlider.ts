...
export function useRangeSlider({
  ...
  const setMinValue = (value: number) =>
    setMinRaw((prev) => {
      const clamped = clamp(value, min, maxValue)
      return clamped === prev ? prev : clamped
    })

  const setMaxValue = (value: number) =>
    setMaxRaw((prev) => {
      const clamped = clamp(value, minValue, max)
      return clamped === prev ? prev : clamped
    })
  ...
  function handleKeyDown(thumb: Thumb) {
    return (e: React.KeyboardEvent): void => {
      const handled: Record<string, () => void> = {
        ...
        PageUp:     () => { if (!stops) moveByStep(thumb, 1, 10) },
        PageDown:   () => { if (!stops) moveByStep(thumb, -1, 10) },
        Home: () => {
          if (thumb === 'min') setMinValue(min)
          else setMaxValue(minValue)
        },
        End: () => {
          if (thumb === 'min') setMinValue(maxValue)
